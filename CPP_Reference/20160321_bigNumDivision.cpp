...
int doTheDivision( int*, int*, int* );
int randomizeArray( int* );
int randomizeArray2( int* );
bool isL1Bigger( int*, int* );

//const int arraySize = 40000000; // this controls the length of the array space
const int arraySize = 40000; // this controls the length of the array space
...
doTheDivision( l1, l2, l3 );

cout << "\nFirst Random Number(" << l1[0] << ") => "; showArray(l1); cout << endl;
cout << "\nSecond Random Number(" << l2[0] << ") => "; showArray(l2); cout << endl;
cout << "\nDivision!! => (" << l3[0] << ") => "; showArray(l3); cout << endl;
...
///-------------------------------------------------------------------------------
int randomizeArray2( int* l1 ) {

	int x;

	l1[0] = 2 + ( rand()%arrayLength ) / 3;
...
int doTheDivision( int* l1, int* l2, int* l3 ) {

	int i;
	int* lx;
	lx = new int [arraySize];
	cout << "\nwe are in doTheDivision.\n";
	// 1. does first character fit?
	// 2. no, equal, call it 1
	// 3. no, less than, move over one and try again
	

	// this next part in non-sense just to see if we are getting to this part
	i = 1;
	if ( l2[l2[0]] < l1[l1[0]] ) { l3[0]++; l3[l3[0]] = 1; }

	return 0;
} // end of doTheDivision
//-------------------------------------------------------------------------------
//-------------------------------------------------------------------------------
//-------------------------------------------------------------------------------
//-------------------------------------------------------------------------------
